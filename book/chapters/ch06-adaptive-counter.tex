...
\chapterlead{Sequential Counter thông thường dành cùng một độ rộng cho mọi
...
trạng thái tổng; Sorting Network các đầu vào; Sequential Counter theo dõi tiền tố. Câu
...
Totalizer & Tổng cây con & \(o_K\land\neg o_{K+1}\) & Dùng lại nhiều cận\\
...
\[
  \sum_{i=1}^{n}x_i=k.
\]
Một Sequential Counter có thể dành \(k\) cột cho mọi tiền tố. Tuy nhiên, sau
\(i\) biến đầu tiên, tổng không thể lớn hơn \(i\). Các ô \(j>i\) vì thế không
mang trạng thái hợp lệ.

\emph{New Sequential Counter} (Sequential Counter mới, NSC) dùng thanh ghi tam giác
\[
  R_{i,j}=1
...
\end{tikzpicture}
\caption{Sequential Counter hình chữ nhật và miền trạng thái tam giác. NSC không
...
Nếu cùng một tập đầu vào cần nhiều cận hoặc cần thay cận tăng dần, Totalizer và
...
với một cận cố định và vùng tam giác bị cắt đủ lớn; Totalizer/mạng đếm phù hợp
