...
ta cần thêm mô hình miền, phép mã hóa, cơ chế tìm cận và một bộ kiểm tra độc lập.
Bốn lớp này tạo thành một thuật toán hoàn chỉnh.}

\index{SAT}\index{DPLL}\index{CDCL}\index{MaxSAT}
\index{incremental SAT}\index{proof logging}\index{model checker}

\flowdiagram{Mô hình miền}{Mã hóa \CNF}{Bộ giải SAT/MaxSAT}
  {Giải mã nghiệm}{Kiểm tra và cận}

\section{Ba dòng nghiên cứu hội tụ}

Phép mã hóa SAT cho tối ưu tổ hợp nằm ở giao điểm của ba truyền thống. Thứ
nhất, \emph{lập luận mệnh đề} (propositional reasoning) đưa bài toán về \CNF
để dùng lan truyền đơn vị, phân tích xung đột và học mệnh đề. Thứ hai,
\emph{lập trình ràng buộc} (constraint programming) nhìn mô hình qua biến miền
hữu hạn, giá trị hỗ trợ và các mức nhất quán. Thứ ba, \emph{tối ưu toán học}
(mathematical optimization) đặt hàm mục tiêu, mô hình nới lỏng, cận dưới và
chứng nhận tối ưu ở vị trí trung tâm.

Các công trình về \emph{nhất quán cung} (arc consistency) trong SAT chỉ ra
rằng chất lượng phép dịch còn phụ thuộc vào các suy luận miền mà bộ lan truyền
tạo được \parencite{gent2002arc,bacchus2007gac}. Một phép mã hóa có thể đúng về
...
\begin{enumerate}
  \item ở cấp mô-đun, xét đặc tả ngữ nghĩa và suy luận do lan truyền đơn vị;
...
với mã hóa trực tiếp, hỗ trợ, thứ tự và lôgarit, cùng nhiều phép phân rã ràng
buộc toàn cục \parencite{tamura2009csp,petke2015bridging}. Trong góc nhìn này,
``mã hóa SAT tối ưu'' là việc chọn một ngôn ngữ Boolean phù hợp với cấu trúc
miền, kiểu suy luận cần giữ và giao diện hàm mục tiêu của bài toán đang xét.

\begin{center}
\captionof{table}{Ba góc nhìn bổ sung khi đánh giá một phép mã hóa SAT.}
...
kích hoạt bằng các giả định (assumptions) hoặc bằng các mệnh đề chỉ bổ sung
\parencite{een2003incremental}.

\begin{designrule}{Bất biến của vòng tìm cận}
Trước khi triển khai, viết đúng một câu định nghĩa \(P(B)\), chiều đơn điệu và
...
\]
Mô hình thỏa ở cận \(7\) là chứng cứ cận trên; chứng minh \UNSAT của \(P(6)\)
là chứng cứ cận dưới. Hai chứng cứ gặp nhau tại giá trị tối ưu \(7\).
\end{workedexample}
...
\begin{tikzpicture}[satfig,x=12mm,y=10mm]
  ...
  \node[satwarn,rounded corners=2pt,above=6mm] at (2,0)
    {\(\UNSAT\) tại \(6\)};
  \node[sattrue,rounded corners=2pt,above=6mm] at (3,0)
    {\(\SAT\) tại \(7\)};
  \draw[satedge] (2,5mm)--(2,1mm);
  \draw[satedge] (3,5mm)--(3,1mm);
...
\emph{mệnh đề} (clause) là phép tuyển các literal; công thức \CNF là phép hội
của các mệnh đề:
\[
...
\parencite{tseitin1968}. Ba mệnh đề trên tạo một \emph{đặc tả ngữ nghĩa}
(contract): \(z\) đúng khi và chỉ khi cả \(x,y\) đúng. Nếu mô-đun sau chỉ dùng chiều
...
đơn vị và học mệnh đề. Thiết kế phép mã hóa vì thế là thiết kế một không gian
...
lại chỉ mã hóa \(z\rightarrow x\land y\), không còn tương đương hai chiều.
...
ghi lý do của mỗi literal trên vết gán (trail), phân tích đồ thị kéo theo, học
một mệnh đề và nhảy lùi đến mức quyết định thích hợp. Khởi động lại (restart)
làm mới phần quyết định nhưng giữ các mệnh đề đã học. Bước chuyển này giải thích vì sao phép mã hóa
không chỉ ảnh hưởng số nút: nó quyết định các cạnh lý do mà phân tích xung đột
có thể sử dụng.

Các bộ giải SAT hiện đại chủ yếu dựa trên \emph{học mệnh đề theo xung đột}
...
Phép mã hóa ảnh hưởng trực tiếp ba lớp của vòng này. Mệnh đề nhị phân và tam
...
Vì vậy hai phép mã hóa có cùng số mệnh đề vẫn có thể khác về thời gian chạy.
Phép mã hóa lớn hơn đôi khi chạy nhanh hơn nhờ các trạng thái trung gian giúp
...
\begin{tikzpicture}[satfig,node distance=12mm and 13mm]
  ...
  \draw[satedge] (l)--(b);
  \draw[satedge] (b)--(d);
  \node[satlabel,below=5mm of l] {Phép mã hóa quyết định độ ngắn của đường suy luận\\
    và phạm vi áp dụng của mệnh đề học được.};
\end{tikzpicture}
\caption{Một vòng CDCL nhìn từ góc độ thiết kế phép mã hóa.}
\label{fig:cdcl-loop}
\end{figure}
...
\MaxSAT bộ phận (Partial MaxSAT) chia công thức thành mệnh đề cứng \(H\) và
...
thường nằm ở \emph{giao diện hàm mục tiêu}, còn phần mã hóa tính khả thi có
thể được dùng chung.

Văn liệu MaxSAT hiện đại thường phân biệt ba kiến trúc. \emph{Tìm kiếm tuyến
tính/nhị phân} đặt cận lên tổng các literal nới lỏng. \emph{Dẫn dắt bằng lõi}
(core-guided) dùng một lõi bất khả thỏa để nhận diện nhóm mệnh đề mềm không thể
đồng thời thỏa, rồi nới chúng có kiểm soát. \emph{Tập đánh trúng}
(hitting-set) tách việc tìm lõi khỏi bài toán chọn các mệnh đề mềm cần vi phạm.
Các bộ giải dạng mô-đun như Open-WBO và RC2 đặt phép mã hóa đếm/PB ở trung tâm
...
\end{enumerate}
Bộ kiểm tra được cài đặt độc lập với bộ mã hóa để hai thành phần tạo thành một phép
kiểm chéo.

Với giá trị tối ưu \(B^\star\), chứng cứ cận trên là một nghiệm có chi phí
...
đơn vị, ghi chính xác mã kiểm tra của \CNF và kiểm chứng minh trên gói tái lập
đó.
...
\midrule
Khả thi tại \(B\) & Mô hình/nghiệm chứng & Kiểm ràng buộc và chi phí miền\\
Vô nghiệm tại \(B\) & Chứng minh mệnh đề & Bộ kiểm chứng trên đúng \CNF\\
Tối ưu \(B^\star\) & Nghiệm tại \(B^\star\) và chứng cứ cận dưới &
Bộ kiểm nghiệm và bộ kiểm chứng cận\\
\bottomrule
\end{tabularx}
\end{center}

\section{Dây chuyền triển khai}
...
\begin{enumerate}
  ...
  \item Lưu nghiệm cùng chứng cứ cận dưới ở cận cuối.
\end{enumerate}
\end{summarybox}

PySAT cung cấp môi trường thuận tiện để thử nghiệm nhiều bộ giải và phép mã hóa
...
\begin{enumerate}
  ...
  \item Chứng nhận tối ưu cần cả nghiệm và chứng cứ cận dưới.
\end{enumerate}

\subsection*{Câu hỏi nghiên cứu}
\begin{enumerate}
  \item Viết \(P(B)\) và quy tắc cập nhật cận cho một bài toán cực đại.
  \item Bỏ từng mệnh đề trong phép mã hóa \(z\leftrightarrow(x\land y)\) và tìm
        phép gán làm mất một chiều tương đương.
  \item Mô tả hai cách mã hóa ``tối thiểu số nhóm được dùng'': SAT theo cận và
        MaxSAT bộ phận (\emph{partial MaxSAT}).
\end{enumerate}
