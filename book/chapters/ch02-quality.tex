\chapter{Thế nào là một phép biểu diễn SAT tốt?}
\chapterlead{Chất lượng phép biểu diễn được đánh giá trên nhiều trục. Đúng đắn là
...
\section{Phép biểu diễn là một quan hệ có giao diện}
...
\begin{description}[style=nextline]
  ...
  \item[Order encoding (biểu diễn thứ tự)]
  ...
  \item[Log/binary encoding (biểu diễn lôgarit/nhị phân)]
...
Biểu diễn lai giữ hai góc nhìn, chẳng hạn biến giá trị để chọn một giá trị và biến
...
\begin{workedexample}{một bất đẳng thức trong biểu diễn trực tiếp và thứ tự}
Cho \(u,v\in\set{0,\ldots,4}\) và \(u+2\le v\). Biểu diễn trực tiếp cấm từng cặp
\((a,b)\) với \(a+2>b\):
\[
  \neg x_{u,a}\lor\neg x_{v,b}.
\]
Biểu diễn thứ tự dùng \(o_{z,t}\leftrightarrow(z\ge t)\) và các mệnh đề
\[
...
\begin{tikzpicture}[satfig,node distance=16mm]
  ...
  \draw[satdash] (w)--(c2);
\end{tikzpicture}
\caption{Đặc tả của literal phụ quyết định cách các mô-đun khác sử dụng literal đó.}
...
Cho phép gán một phần \(A\). \emph{Unit propagation} (lan truyền đơn vị, UP)
lặp lại quá trình thỏa mãn mệnh đề
đơn vị cho
...
Trong một phép biểu diễn miền hữu hạn, \emph{generalized arc consistency}
...
đại diện trực tiếp cho khả năng duy trì giá trị trong miền, có thể so sánh bao đóng
...
\section{Cận dưới kích thước và biên Pareto (Pareto frontier)}

Yêu cầu lan truyền đặt ra cận dưới cho kích thước phép biểu diễn. Với AMO, các
cận dưới đã biết cho thấy một phép biểu diễn đầy đủ lan truyền cần số mệnh đề
tuyến tính với hệ số bị chặn, kể cả khi dùng biến phụ
\parencite{kucera2019lower}. Kết quả này dẫn đến ba nguyên tắc:
\begin{enumerate}
  \item xác định rõ lớp biểu diễn khi phát biểu về kích thước, biến phụ và sức lan
        truyền;
  \item một cải tiến vài mệnh đề có thể đáng kể nếu mức giảm đó tiến gần cận dưới lý thuyết;
...
bộ lan truyền đạt mức nhất quán mạnh hơn phép biểu diễn đa thức đang xét, phép so
sánh tập trung vào phần suy luận được chuyển sang học mệnh đề và chi phí
để đạt phần suy luận đó.

\begin{keyidea}{Biên Pareto (Pareto frontier) của phép biểu diễn}
Một bộ biểu diễn được định vị trên bốn trục: kích thước, sức lan truyền, chi phí
...
thực nghiệm của từng thiết kế trên biên Pareto (Pareto frontier) này.
\end{keyidea}

\section{Mức độ phù hợp với bộ giải}

Hai phép biểu diễn có cùng tính đúng và kích thước vẫn có thể khác về:
\begin{itemize}
...
\section{Tính mô-đun và biểu diễn lại có cấu trúc}

Một mô-đun biểu diễn tốt có bốn thành phần:
\[
  \texttt{variables}
  +\texttt{clauses}
  +\texttt{decoder}
  +\texttt{contract}.
\]
Khi nhiều ràng buộc chia sẻ biểu thức con, biểu diễn lại có cấu trúc tạo biến phụ
...
\begin{center}
\captionof{table}{Ma trận phát biểu--bằng chứng cho một phép biểu diễn SAT.}
\label{tab:claim-evidence}
\begin{tabularx}{\textwidth}{@{}>{\bfseries\sffamily}p{.24\textwidth}YY@{}}
\toprule
Phát biểu & Bằng chứng lý thuyết & Bằng chứng thực nghiệm\\
\midrule
Biểu diễn đúng & Chứng minh hai chiều & Kiểm thử vét cạn/vi sai trên miền nhỏ\\
Biểu diễn nhỏ hơn & Công thức đếm & Kích thước \CNF thực tế\\
...
\section{Một quy trình đánh giá phép biểu diễn}
...
\begin{enumerate}
  ...
  \item Thời gian chạy là kết quả của tương tác giữa phép biểu diễn, bộ giải và bài toán thử.
\end{enumerate}

\subsection*{Câu hỏi nghiên cứu}
\begin{enumerate}
  \item Cho một phép biểu diễn AMO có biến phụ, viết bộ giải mã và chứng
        minh theo phép chiếu.
  \item Thiết kế một phép thử phân biệt đặc tả điều kiện đủ với đặc tả tương
        đương của thanh ghi ngưỡng.
  \item Chọn ba thước đo để giải thích vì sao một phép biểu diễn lớn hơn lại chạy
