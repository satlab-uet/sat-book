...
\begin{itemize}
  \item \textbf{tính đúng} (soundness): mọi mô hình thỏa của \(F\) chiếu thành
        nghiệm của \(\mathcal C\);
  \item \textbf{tính đầy đủ} (completeness): mọi nghiệm của \(\mathcal C\) mở
...
Với bộ đếm, cách xây tự nhiên là đặt thanh ghi ngưỡng đúng theo tổng tiền tố
thật.
...
\begin{description}[style=nextline]
  ...
  \item[Mã hóa thứ tự (order encoding)]
  ...
  \item[Mã hóa lôgarit/nhị phân (log/binary encoding)]
...
\section{Đặc tả ngữ nghĩa của biến phụ}
...
Một mô-đun nối dùng \(\neg r_{i,j}\) như mệnh đề ``tổng dưới \(j\)''
phải có chiều cần. Nếu bộ đếm chỉ cung cấp chiều đủ, bộ nối đó không đúng
dù khi đứng riêng hai công thức vẫn có thể \emph{đồng khả thỏa}
(tức cùng có nghiệm hoặc cùng vô nghiệm).
...
\section{Lan truyền đơn vị}

Cho phép gán bộ phận \(A\). Lan truyền đơn vị (UP) lặp việc thỏa một mệnh đề
đơn vị cho
...
\begin{description}[style=nextline]
  ...
  \item[Đầy đủ bác bỏ bằng lan truyền đơn vị (URC)]
  Mọi phép gán làm \(F\) không thỏa được đều bị UP chứng minh bất khả thỏa
  trên toàn bộ biến. Tính chất này thường được gọi là
  \emph{unit-refutation completeness}.

  \item[Đầy đủ lan truyền (PC)]
  Mọi literal bị \(F\land A\) kéo theo trong phạm vi quan sát đều được UP suy
  ra, hoặc UP chứng minh bất khả thỏa.
\end{description}

Trong một phép mã hóa miền hữu hạn, \emph{nhất quán cung tổng quát}
(generalized arc consistency, GAC) loại mọi giá trị không còn giá trị hỗ trợ.
...
sẽ đánh đổi ít nhất một trục khác. Bộ kiểm chuẩn đo vị trí thực nghiệm
của từng thiết kế trên mặt Pareto này.
\end{keyidea}
...
Nhanh hơn & Phân tích cơ chế & Bộ kiểm chuẩn, nhiều hạt giống, hồ sơ hiệu năng\\
...
\begin{enumerate}
  ...
  \item Xây dựng bộ đối chiếu nhỏ (\emph{oracle}) để so sánh tập nghiệm sau phép chiếu.
