...
\subsection{Quan hệ với biểu diễn hỗ trợ}

Trong biểu diễn trực tiếp/hỗ trợ, lựa chọn \(x_{u,a}\) yêu cầu \(v\) nhận một
...
kiểm tra; nó xác định toàn bộ cấu trúc liên kết của phép biểu diễn. Các cặp có
...
\[
  \neg x_{u,a}\lor\neg x_{v,b}.
\]
Biểu diễn trực tiếp có phép giải mã đơn giản nhưng số xung đột tăng theo độ rộng
khoảng cấm.
...
\paragraph{Direct Encoding (biểu diễn trực tiếp, DE).}
...
\paragraph{Forward Order Encoding (biểu diễn thứ tự xuôi, FE).}
\(X_{v,j}=1\iff L_v\ge j\). Giá trị chính xác xuất hiện ở điểm chuyển và kích
hoạt hai biên của khoảng \([j-k+1,j+k-1]\).

\paragraph{Reverse Order Encoding (biểu diễn thứ tự ngược, RE).}
