...
\paragraph{Direct Encoding (mã hóa trực tiếp, DE).}
...
\paragraph{Forward Order Encoding (mã hóa thứ tự xuôi, FE).}
\(X_{v,j}=1\iff L_v\ge j\). Giá trị chính xác xuất hiện ở điểm chuyển và kích
hoạt hai biên của khoảng \([j-k+1,j+k-1]\).

\paragraph{Reverse Order Encoding (mã hóa thứ tự ngược, RE).}
