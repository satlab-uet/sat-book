...
\chapterlead{Gán nhãn radio, gán nhãn an toàn \(k\) và phân bổ tần số cùng chia
sẻ một mô-đun: một lựa chọn chính xác tạo ra một khoảng cấm trên miền của
đối tượng khác. Sự khác nhau nằm ở cách tạo khoảng cấm và cách định nghĩa hàm
mục tiêu.}
...
\[
  ...
  f(v)>U(a).
\]
Trên miền nguyên liên tục, nó trở thành
\[
  \neg x_{u,a}
  \lor
  \neg g_{v,L(a)}
...
\subsection{Quan hệ với mã hóa hỗ trợ}

Trong mã hóa trực tiếp/hỗ trợ, lựa chọn \(x_{u,a}\) yêu cầu \(v\) nhận một
...
tiền tố và một hậu tố. Nó không phải một mẹo riêng của radio/FAP; đó là phép
...
kết giá trị chính xác--thứ tự. Hai phép mã hóa đạt cùng mức cắt tỉa khi đặc tả của các
...
\section[Gán nhãn radio k]{Gán nhãn radio \(k\)}
...
kiểm tra; nó xác định toàn bộ cấu trúc liên kết của phép mã hóa. Các cặp có
...
\[
  \neg x_{u,a}\lor\neg x_{v,b}.
\]
Mã hóa trực tiếp có phép giải mã đơn giản nhưng số xung đột tăng theo độ rộng
khoảng cấm.
...
gán nhãn đơn ánh; nó không phải tô màu đồ thị chuẩn. Biến thể không đơn ánh là
một mô hình khác.
\end{designrule}

\subsection{Mã hóa trực tiếp, thứ tự xuôi và thứ tự ngược}

\paragraph{Mã hóa trực tiếp (DE).}
...
đại diện và đặt nó ở nửa dưới miền để giữ một đại diện của cặp phản xạ.

\begin{resultbox}{So sánh ba phép mã hóa}
...
Đây là AC-3 trên các ràng buộc phân cách nhị phân. Độ mạnh của nó giới hạn ở
...
Mã hóa SAT trực tiếp dùng \(x_{v,q}=1\iff f_v=D_{v,q}\), ràng buộc đúng một
theo máy phát và xung đột từng cặp cho các cặp giá trị vi phạm.

Mã hóa SAT thứ tự bộ phận thêm
\[
  g_{v,q}=1\iff
...
\begin{tikzpicture}[satfig,x=13mm,y=9mm]
  ...
  \draw[Gold,decorate,decoration={brace,amplitude=4pt}] (1.55,-.55)--(3.45,-.55)
    node[midway,below=5pt,satlabel,text=Gold]
...
\[
  \min\sum_aA_a.
\]
Bộ đếm tuần tự tăng dần được dựng một lần đến cận trên ban đầu. Sau khi mô hình
...
Ba bài toán dùng chung mô-đun khả thi ở mức khoảng, nhưng lớp hàm mục tiêu khác
...
ưu ràng buộc, tìm kiếm cục bộ và tìm kiếm kiêng kỵ (\emph{tabu}) là các đối
...
\end{itemize}
Bộ kiểm tra không đọc các biến thứ tự phụ. Điều này giữ nó nhỏ và độc lập với
phép mã hóa.

Chứng nhận tối ưu gồm một nghiệm chứng tại cận trên và bằng chứng cận dưới:
\UNSAT ở độ trải/số giá trị kề trước, hoặc một cận dưới toán học bằng cận trên.
Với các giả định tăng dần, có thể vật hóa các giả định cuối thành mệnh đề đơn
...
\begin{enumerate}
  ...
  \item Biến trực tiếp phục vụ lựa chọn chính xác và giải mã; biến thứ tự nén
        khoảng; bộ đếm tuần tự đếm số giá trị được dùng.
\end{enumerate}
\end{summarybox}
