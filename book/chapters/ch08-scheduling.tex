...
\chapterlead{Một phép mã hóa cho bài toán thời gian chỉ thực sự rõ khi nó trả lời
...
\section{Từ SAT theo mốc thời gian đến sinh mệnh đề lười}
...
\begin{enumerate}
  ...
  \item tích hợp lan truyền miền với học mệnh đề, điển hình là sinh mệnh đề
        lười (\emph{lazy clause generation}, LCG).
\end{enumerate}
...
Vì vậy, một phép mã hóa lập lịch được đánh giá trên cả kích thước, sức lan
...
lịch tàu. Tuy nhiên, phép mã hóa của chúng đều có thể tách thành bốn lớp:
\begin{enumerate}
...
máy và thay phép mã hóa cho tính không ngắt; hoặc giữ nguyên mô hình khả thi
...
đoạn. Tính không ngắt (\emph{non-preemption}) đòi hỏi mạnh hơn:
\[
  z_{it}=1 \iff s_i\le t<s_i+e_i .
\]

Mã hóa trực tiếp có thể tạo một biến \(b_{is}\) cho từng thời điểm bắt đầu
...
\(\bigvee_{s\in T_i}b_{is}\); phần nhiều nhất một được sinh bằng mã hóa từng
...
Phép mã hóa gọn thay phần lặp bằng các biến tiền tố/hậu tố có ngữ nghĩa ``tất
...
\begin{proposition}[Đúng đắn của phép mã hóa theo khối]
...
ba mốc rời; tính không ngắt phải mã hóa vị trí bắt đầu và hai biên khối.}
...
hóa gọn nằm ở việc tránh lặp cấu trúc liên tục. Cấu hình kết hợp mã hóa theo
khối với phá đối xứng và một phép mã hóa giả Boolean thích hợp đạt tổng thời
...
\]
Nếu mã hóa từng cửa sổ bằng một phép mã hóa ràng buộc đếm độc lập, hai cửa sổ
...
tuần; ưu tiên tạo các chi phí mềm. Phép mã hóa cửa sổ dùng chung tác động trực
...
Trên 28 bài toán lập lịch điều dưỡng (\emph{Nurse Rostering}), với quy mô đến
...
\section{Cân bằng dây chuyền với đỉnh công suất}
...
Cùng một phép mã hóa tính khả thi có thể kết nối với hàm mục tiêu theo bốn cách:
\begin{center}
...
cứu của \textcite{kieu2025power}, phép mã hóa tuần tự gọn kết hợp giải tăng dần
...
hơn và thí nghiệm bóc tách rõ hơn.
\end{keyidea}
...
Khám phá rời rạc hóa động (\emph{Dynamic Discretization Discovery}, DDD) khởi
...
vào SAT, thay vì chỉ thay phép mã hóa của một ràng buộc.
...
\begin{enumerate}
  ...
  chứng cứ \(C_r\) chỉ ra phần rời rạc hóa đang thiếu cho feasibility hoặc
  hàm mục tiêu.
  ...
  \(\Lambda^\star\setminus\Lambda^r\) được chứng cứ \(C_r\) yêu cầu; đặt
...
tập mốc đầy đủ---xác định đồng thời tính đúng đắn và tốc độ hội tụ.
...
không đạt được bị loại ngay. AMO kết hợp tiếp tục chọn phép mã hóa khác nhau
...
Trên các bài toán của bản thảo \textcite{kieu2026train}, hiệu quả thay đổi theo
...
\begin{enumerate}
  \item Với cùng ngữ nghĩa khối, hãy so sánh mã hóa bằng biến bắt đầu và mã hóa
