...
\begin{tikzpicture}[satfig,x=8.5mm,y=7.5mm]
  \node[anchor=west] at (0,3.6) {(a) Điểm cắt miền tiền nhiệm};
  \node[anchor=east] at (0.8,2.4) {$A_i$};
  \foreach \x/\v in {1/1,2/3,3/4,4/7} {
    \draw[satbox] (\x*1.1,2.1) rectangle +(0.95,0.65);
    \node at (\x*1.1+0.475,2.425) {$\v$};
  }
  \draw[Gold,dashed,very thick] (2.72,1.95)--(2.72,2.85);
  \node[Gold,anchor=south,font=\rmfamily\bfseries\footnotesize] at (2.72,2.85) {$c=2$};
  \node[anchor=west] at (4.8,2.4) {$x_{j,4}$};
  \node[anchor=west] at (0.8,1.2) {$\neg x_{j,4} \lor \neg q_{i,2}$};
  \node[anchor=west] at (0.8,0.5) {$q_{i,2} = x_{i,4} \lor x_{i,7}$};
\end{tikzpicture}
\end{subfigure}%
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\centering
\begin{tikzpicture}[satfig,x=8.5mm,y=7.5mm]
  \node[anchor=west] at (0,3.6) {(b) Đánh giá ngưỡng chờ nội bộ};
  \node[anchor=east] at (0.8,2.4) {mốc};
  \foreach \x/\v in {1/3,2/4,3/5,4/6} {
    \draw[satbox] (\x*1.1,2.1) rectangle +(0.95,0.65);
    \node at (\x*1.1+0.475,2.425) {$\v$};
  }
  \fill[Blue!80] (1.1+0.2,2.25) rectangle +(0.55,0.35);
  \fill[Blue!80] (3.3+0.2,2.25) rectangle +(0.55,0.35);
  \node[anchor=south,font=\rmfamily\footnotesize] at (1.1+0.475,2.8) {$f_{p,3}$};
  \node[anchor=south,font=\rmfamily\footnotesize] at (3.3+0.475,2.8) {$R_{p,5}$};
  \draw[satdim] (1.1+0.475,1.75)--(3.3+0.475,1.75);
  \node[anchor=north,font=\rmfamily\footnotesize] at (2.2+0.475,1.7) {$\card{M_p}=2 \Rightarrow I_p \ge 1$};
\end{tikzpicture}
\end{subfigure}
...
\]
Gọi \(\theta_{p,k}\) là biến mệnh đề đại diện cho điều kiện \(I_p(S)\ge k\). Mối liên hệ giữa mốc đầu và mốc cuối được mã hóa chính xác bằng:
\begin{equation}
