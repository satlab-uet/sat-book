\chapter{Thiết kế thực nghiệm cho phép mã hóa SAT}
\chapterlead{Đánh giá trên bộ kiểm chuẩn không chỉ trả lời bộ mã hóa nào nhanh
...
Một bảng thời gian chạy không tự nói bộ mã hóa nào tốt hơn. Quan sát được tạo
bởi cả
chuỗi
\[
  \text{bài toán thử}
  \rightarrow\text{mô hình}
  \rightarrow\text{bộ mã hóa}
...
\emph{quần thể} (\emph{population}) mà nó hướng đến: một họ bài toán thử, một
miền tham số, một bộ giải hay một danh mục bộ giải. Cần phân biệt
\emph{hiệu lực nội tại} (\emph{internal validity})---hiệu ứng thật sự do yếu tố
đang xét---với \emph{hiệu lực khái quát} (\emph{external validity})---hiệu ứng
có còn giữ ngoài tập kiểm chuẩn hay không.

Trong nghiên cứu phép mã hóa, ba loại bằng chứng bổ sung nhau:
\begin{itemize}
  \item \textbf{bằng chứng cơ chế}: công thức đếm, vết lan truyền và xung đột
        nhỏ giải thích vì sao hiệu ứng xuất hiện;
  \item \textbf{bằng chứng có kiểm soát}: thí nghiệm bóc tách
        (\emph{ablation}) giữ cố định các yếu tố khác;
...
Một nghiên cứu về phép mã hóa thường có một trong bốn câu hỏi:
\begin{enumerate}
  \item phép mã hóa mới giảm kích thước bao nhiêu;
...
phá đối xứng, kết quả không cho biết hiệu ứng thuộc về đâu.

\begin{designrule}{Đơn vị của thí nghiệm bóc tách}
Một đặc trưng không có hiệu ứng độc lập khi nó thay đổi giao diện của phép mã hóa.
Trong trường hợp đó, dùng cấu hình
\(\text{phép mã hóa}\times\text{đặc trưng}\) làm đơn vị so sánh.
\end{designrule}
...
Mọi cấu hình phải dùng cùng ngữ nghĩa và bộ kiểm tra. Nếu một bộ mã hóa có tiền
...
một phép mã hóa thắng trong một vùng.
...
Tùy chọn bộ giải, độ rộng khối, kích thước nhóm và ngưỡng mã hóa là các siêu
...
\begin{enumerate}
  ...
  \item nếu dữ liệu ít, dùng kiểm định chéo (\emph{cross-validation}) theo họ
...
\subsection{Thời gian chạy có kiểm duyệt}

Khi một lần chạy hết thời hạn, ta chỉ biết thời gian cần thiết lớn hơn giới hạn;
dữ liệu như vậy được gọi là \emph{kiểm duyệt phải} (\emph{right-censored}).
...
\begin{itemize}
  ...
  \item biểu đồ xương rồng (\emph{cactus plot});
  \item hồ sơ hiệu năng (\emph{performance profile});
...
\end{tikzpicture}
\caption{Hồ sơ hiệu năng dạng bậc thang: phần trái cho số lần thắng, phần đuôi
cho độ bền so với cấu hình tốt nhất trên từng bài toán thử.}
\label{fig:performance-profile}
\end{figure}

\subsection{Biểu đồ xương rồng và biểu đồ phân tán}

Với một cấu hình \(s\), sắp tăng thời gian của các bài đã giải:
\[
  t_{(1),s}\le t_{(2),s}\le\cdots\le t_{(q_s),s}.
\]
Biểu đồ xương rồng đặt số bài đã giải \(j\) trên trục ngang và
...
nào} tạo khác biệt; biểu đồ xương rồng cho biết \emph{toàn tập} được bao phủ
đến đâu.
...
\begin{tikzpicture}[satfig,x=8mm,y=8mm]
  \begin{scope}
    ...
    \node[satlabel,font=\sffamily\bfseries\small] at (2.5,4.6) {Xương rồng};
  \end{scope}
  ...
  \end{scope}
\end{tikzpicture}
\caption{Biểu đồ xương rồng (trái) giữ số bài giải được trong hình; biểu đồ
...
Các bài toán SAT cần tìm một nghiệm chứng; các bài toán UNSAT cần đóng toàn bộ
không gian tìm kiếm. Một phép mã hóa có thể mạnh ở SAT nhưng yếu ở UNSAT hoặc
ngược lại.
...
\section{Nhiều hạt giống và biến thiên}

CDCL có yếu tố ngẫu nhiên trong phá hòa và tìm kiếm. Với bài toán khó, cần chạy
nhiều hạt giống và báo trung vị, các phân vị hoặc khoảng tin cậy. So sánh ghép
cặp phải dùng cùng bài toán, cùng chỉ số hạt giống và cùng giới hạn thời gian,
bộ nhớ.

Nếu bộ mã hóa là tất định nhưng bộ giải có yếu tố ngẫu nhiên, hạt giống thuộc
về bộ giải. Nếu bộ sinh dùng thứ tự ngẫu nhiên, cần tách hạt giống của bộ sinh
khỏi hạt giống của bộ giải.

\section{So sánh ghép cặp và độ lớn hiệu ứng}

Mỗi bài toán thử tạo một cặp quan sát tự nhiên giữa hai bộ mã hóa. Vì phân bố
...
\begin{itemize}
  \item số lần thắng/hòa/thua trên cùng bài toán và cùng hạt giống;
  \item trung vị của lôgarit hệ số tăng tốc
        \(\log(t_{\mathrm{base}}/t_{\mathrm{new}})\);
  \item khoảng bất định bằng lấy mẫu lại bootstrap theo \emph{họ}, không chỉ
        theo từng bài toán;
  \item chênh lệch số bài giải được tại nhiều mốc thời gian.
\end{itemize}

Độ lớn hiệu ứng quan trọng hơn một giá trị \(p\) đứng riêng. Một bộ mã hóa nhanh
hơn \(3\%\) trên hàng nghìn bài dễ nhưng làm mất năm bài khó có ý nghĩa khác
với bộ mã hóa chỉ giảm ít trung vị nhưng mở rộng vùng giải được. Với tối ưu,
...
khi \(k/n\) nhỏ; mạng đếm có thể cải thiện khi cần nhiều ngưỡng; mã hóa thứ tự
...
\begin{enumerate}
  ...
  \item kiểm thử vi sai giữa hai bộ mã hóa;
...
Một gói phần mềm--dữ liệu tái lập (\emph{artifact}) tối thiểu gồm:
\begin{itemize}
...
Số liệu thực nghiệm trong một khảo cứu được gắn với ba tầng bằng chứng:
\begin{enumerate}
  \item \textbf{được tác giả nguồn báo cáo}: giữ đúng tập bài toán thử, giới
        hạn thời gian và cấu hình đối chứng;
  \item \textbf{đối chiếu được từ gói tái lập}: có đủ mã nguồn, nhật ký và dữ
        liệu để tái tạo bảng;
  \item \textbf{được tái lập độc lập}: chạy lại bằng môi trường do người tổng
        hợp kiểm soát.
\end{enumerate}
Mỗi hộp kết quả trong sách ghi công trình nguồn và phạm vi thí nghiệm; khi hồ
sơ có gói tái lập, đường dẫn mã nguồn, nhật ký và dữ liệu được quản lý trong sổ
nguồn. Ba tầng này tạo lộ trình nâng một kết quả đã công bố thành một kết quả
tái lập độc lập.
...
\textbf{Yếu tố.} Bộ mã hóa, bộ giải, đặc trưng, họ bài toán, hạt giống.\\
...
\begin{enumerate}
  ...
  \item Tách SAT/UNSAT và dùng nhiều hạt giống ở vùng khó.
  \item Bộ kiểm tra mô hình là điều kiện trước của mọi bảng hiệu năng.
\end{enumerate}

\subsection*{Câu hỏi nghiên cứu}
\begin{enumerate}
  \item Thiết kế thí nghiệm bóc tách \(2\times2\) cho bộ mã hóa và phá đối xứng.
