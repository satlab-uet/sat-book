...
\subsection{Từ bùng nổ mệnh đề từng cặp đến mã hóa khoảng bằng biến tiền tố}

Chuyển trực tiếp Ràng buộc khoảng cách~\eqref{eq:nohole-ilp-distance} sang \CNF bằng phép mã hóa từng cặp (Direct Encoding - DE) sinh ra họ mệnh đề xung đột:
\begin{equation}
...
Để giải quyết triệt để nút thắt này, phép mã hóa \mbox{\textnormal{\textsc{PSE}}} (\emph{Proposed SAT Encoding}) \parencite{hai2026nohole} giới thiệu các biến phụ tiền tố \(l_{ij} \in \set{0,1}\) với ngữ nghĩa:
\[
...
\begin{tikzpicture}[satfig,x=8.5mm,y=7.5mm]
  %--- Direct Encoding ---
  \node[anchor=west,font=\rmfamily\bfseries\small] at (0,3.5) {(a) Phép mã hóa từng cặp (Direct Encoding)};
  ...
  \node[anchor=west,font=\rmfamily\bfseries\small] at (7.8,3.5) {(b) Phép mã hóa tiền tố khoảng cấm (PSE)};
...
\end{tikzpicture}
\caption{So sánh cơ chế nén khoảng cấm khi $k=5, w=2$: (a) Phép mã hóa từng cặp liệt kê $O(w)$ mệnh đề; (b) Phép mã hóa tiền tố PSE loại toàn bộ khoảng cấm bằng một mệnh đề kéo theo duy nhất.}
\label{fig:nohole-prefix-interval}
\end{figure}

\begin{proposition}[Độ phức tạp và tính đúng đắn của PSE]
\label{prop:nohole-pse-complexity}
Phép biểu diễn \mbox{\textnormal{\textsc{PSE}}} biểu diễn chính xác bài toán No-hole Anti-\(k\)-labeling. Tổng số biến nhị phân là \(2nk\), tổng số mệnh đề khoảng cách là \(2\card{E}k\), và các mệnh đề liên kết là \(n(4k-2)\). Phép mã hóa hoàn toàn triệt tiêu sự phụ thuộc của số mệnh đề vào tham số khoảng cách mục tiêu \(w\).
\end{proposition}
