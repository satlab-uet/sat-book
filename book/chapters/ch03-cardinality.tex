...
\chapterlead{Ràng buộc đếm (\emph{cardinality constraint}) giới hạn số literal
đúng trong một tập; ràng buộc giả Boolean (\emph{pseudo-Boolean}) mở rộng nó
bằng các trọng số nguyên. Hai lớp này xuất hiện trong ràng buộc gán, sức chứa,
chuỗi, đối xứng và hàm mục tiêu. Không có một bộ mã hóa tốt nhất cho mọi miền;
...
\flowdiagram{AMO/EXO}{AMK/ALK}{Đúng \(K\)}
  {Giả Boolean}{Chọn bộ mã hóa}

\section{Bản đồ phát triển của phép mã hóa ràng buộc đếm}

Các phép mã hóa ràng buộc đếm không phát triển theo một chuỗi trong đó bộ mã
hóa mới thay thế hoàn toàn bộ mã hóa cũ. Mỗi họ giải quyết một kiểu đánh đổi:
\begin{enumerate}
  \item mã hóa từng cặp (\emph{pairwise}) và các phép phân rã AMO ưu tiên
        phát hiện xung đột trực tiếp;
  \item bộ đếm tuần tự (\emph{sequential counter}) vật hóa tổng trên từng tiền tố;
  \item cây tổng (\emph{totalizer}) vật hóa tổng theo cấu trúc cây;
  \item mạng sắp xếp/lựa chọn (\emph{sorting/selection network}) ưu tiên cấu
        trúc đều và độ sâu nhỏ;
  \item biểu đồ quyết định nhị phân (BDD) hợp nhất các trạng thái tổng tương đương;
  \item cây tổng tổng quát và các biến thể PB xử lý trọng số;
  \item danh mục phương pháp hoặc bộ chọn đã học chọn phép mã hóa theo đặc
...
một bộ giải cụ thể \parencite{bailleux2010survey}. Một bộ đếm tuần tự
\(O(nk)\) có thể nhỏ khi \(k\) bé; một mạng đếm có thể lớn hơn nhưng có độ sâu
tốt; cây tổng lại thuận lợi khi cận thay đổi. Không có quan hệ thứ tự tuyến
tính giữa ba lựa chọn đó.

\section{Các dạng ràng buộc}

Với \(X=\set{x_1,\ldots,x_n}\):
\[
...
\]
AMO là AMK với \(k=1\); ALO là một mệnh đề dài
\(x_1\lor\cdots\lor x_n\); EXO là AMO kết hợp ALO.
...
mã hóa trực tiếp ALK và mã hóa AMK trên các literal phủ định phụ thuộc vào
\(k\) và những đầu ra đã có.

\section{Ràng buộc nhiều nhất một (AMO)}

\subsection{Mã hóa từng cặp}

Mã hóa từng cặp sinh
\[
  \neg x_i\lor\neg x_j
  \qquad\forall i<j.
\]
Nó không dùng biến phụ, có \(\binom n2\) mệnh đề nhị phân và lan truyền trực
tiếp. Đây là lựa chọn tốt khi miền nhỏ.

\subsection{Mã hóa tuần tự}

Đặt \(s_i=1\) khi ít nhất một trong \(x_1,\ldots,x_i\) đúng. Một biến thể
mã hóa AMO tuần tự dùng:
\[
  \neg x_i\lor s_i,
  \qquad
  \neg s_{i-1}\lor s_i,
...
\subsection{Mã hóa tích và mã hóa chỉ huy}

Mã hóa tích (\emph{product encoding}) đặt biến vào ma trận rồi dùng AMO theo
hàng và cột. Mã hóa chỉ huy (\emph{commander encoding}) chia biến thành nhóm,
tạo một biến đại diện cho mỗi nhóm rồi mã hóa AMO trên các đại diện. Hai cách
này tạo tính cục bộ theo khối và phù hợp với miền vừa hoặc lớn
\parencite{chen2010amo}.

\begin{center}
\captionof{table}{So sánh các bộ mã hóa AMO cơ bản.}
\label{tab:amo-encoders}
\begin{tabularx}{\textwidth}{@{}>{\bfseries\sffamily}p{.19\textwidth}YYY@{}}
\toprule
Phép mã hóa AMO & Biến phụ & Mệnh đề & Điểm mạnh\\
\midrule
...
\section{Bộ đếm tuần tự cho AMK}
...
Nếu các hệ thức truy hồi được mã hóa hai chiều thì, với mọi \(i,j\),
\[
...
\end{tikzpicture}
\caption{Lưới trạng thái của bộ đếm tuần tự cho \(n=5,k=2\). Mũi tên liền
...
suy ra \(\neg x_5\). Bộ đếm không cần đợi đầu vào đúng thứ ba mới báo xung đột; nó
loại mọi vị trí còn lại ngay khi cận đã đầy.
\end{workedexample}

\section{Cây tổng}

Cây tổng xây dựng một cây nhị phân. Mỗi nút trả về một vectơ đơn phân
\[
...
\begin{tikzpicture}[satfig,level distance=13mm,
  ...
  \draw[decorate,decoration={brace,amplitude=4pt},Ink]
    ([xshift=-7mm]a.west)--([xshift=7mm]b.east)
    node[midway,above=6pt,satlabel]{ghép hai vectơ đơn phân};
\end{tikzpicture}
\caption{Cây tổng cân bằng: mỗi nút trả về các ngưỡng của tổng trong cây con.}
\label{fig:totalizer-tree}
\end{figure}

\section{Mạng đếm}

Mạng sắp xếp sắp các đầu vào Boolean thành các đầu ra đơn phân:
\[
...
\begin{tikzpicture}[satfig,x=17mm,y=9mm]
  ...
  \draw[satedge] (d)--(lo);
\end{tikzpicture}
\caption{Bộ so sánh Boolean là viên gạch của mạng sắp xếp và mạng đếm.}
\label{fig:boolean-comparator}
\end{figure}

\section{Mã hóa bằng BDD}

Biểu đồ quyết định nhị phân (\emph{binary decision diagram}, BDD) biểu diễn
...
mỗi nút BDD cùng các mệnh đề liên kết theo hai cung tạo thành phép mã hóa
...
\section{Ràng buộc đúng \(K\) và đầu ra dùng chung}

EXK có thể viết thành AMK và ALK, nhưng dựng hai bộ đếm độc lập sẽ lặp trạng
thái. Nếu bộ mã hóa trả về các đầu ra đơn phân \(o_j\), chỉ cần
\[
  o_k\land\neg o_{k+1}.
\]
Bộ đếm tuần tự cũng có thể dùng chung thanh ghi: biên \(s_{n,k}\) bảo đảm ít
...
\begin{itemize}
  \item mã hóa bằng bộ cộng hoặc mạch cho biểu diễn nhị phân của tổng;
  \item BDD cho các trạng thái tổng có thể đạt;
  \item cây tổng tổng quát gom và cắt các tổng;
  \item phân rã bằng mạng sắp xếp hoặc mạng đếm sau khi tách trọng số;
  \item thư viện kết hợp như PBLib chọn bộ mã hóa theo cấu trúc
...
\subsection{Cây tổng tổng quát}

Cây tổng tổng quát (\emph{generalized totalizer}) thay vectơ đơn phân liên tiếp
...
\]
trong đó \(B+1\) gom mọi trạng thái tràn. Phép mã hóa đặc biệt hiệu quả khi
...
như các biến độc lập, bộ mã hóa sẽ bỏ mất thông tin này. Các phép mã hóa
PB(AMO) đưa nhóm AMO trực tiếp vào biểu đồ quyết định đa trị (MDD), cây tổng
tổng quát, bộ đếm tuần tự có trọng số hoặc cây tổng theo môđun
...
Đây là một ví dụ tổng quát của \emph{phép mã hóa có xét cấu trúc}
(\emph{structure-aware encoding}): trước khi chọn bộ mã hóa cho một biểu thức
...
Quy tắc ``\(k\) nhỏ thì dùng bộ đếm tuần tự, nhiều cận thì dùng cây tổng'' là
một đối chứng hữu ích, nhưng chưa bao quát tương tác giữa hệ số, nhóm AMO, độ
rộng miền và bộ giải. Nghiên cứu về lựa chọn thuật toán
(\emph{algorithm selection}) đã dùng các đặc trưng của ràng buộc và bài toán
thử để chọn giữa nhiều phép mã hóa SAT, kể cả trên những lớp bài toán chưa xuất
hiện trong dữ liệu huấn luyện \parencite{ulrich2022selection}. Kết quả này thay
đổi cách xây dựng thư viện bộ mã hóa: mỗi mô-đun cần công bố đủ siêu dữ liệu để
một bộ chọn ra quyết định.

\begin{center}
\captionof{table}{Nhóm đặc trưng để lựa chọn bộ mã hóa theo bài toán thử.}
...
\midrule
Kích thước & \(n,k,B\), số hạng tử & Loại bộ mã hóa chắc chắn quá lớn\\
...
Giao diện & Số cận, thêm đầu vào, vật hóa & Giải tăng dần và kết hợp mô-đun\\
Mô hình & Mật độ, đối xứng, đồ thị ràng buộc & Tương tác với học mệnh đề\\
\bottomrule
\end{tabularx}
\end{center}

Bộ chọn không loại bỏ nhu cầu chứng minh tính đúng: nó chỉ chọn giữa các
...
bộ kiểm chuẩn thay vì học cấu trúc của phép mã hóa.

\section{Bản đồ lựa chọn}

\begin{center}
\captionof{table}{Gợi ý lựa chọn bộ mã hóa ràng buộc đếm/PB.}
...
Nhiều ngưỡng & Cây tổng/mạng đếm & Tái sử dụng đầu ra\\
...
PB có nhóm AMO & Bộ mã hóa PB(AMO) & Không vật hóa trạng thái bất khả\\
Cận thay đổi tăng dần & Cây tổng/đầu ra thứ tự & Giao diện cận ổn định\\
\bottomrule
\end{tabularx}
\end{center}

Đây là cấu hình khởi đầu; Chương 4 trình bày cách đánh giá bộ mã hóa trên phân
...
\begin{enumerate}
  \item Chọn bộ mã hóa theo cận và đầu ra cần đọc, không chỉ theo tên ràng buộc.
  \item Mã hóa AMO từng cặp vẫn là đối chứng mạnh khi miền nhỏ.
  \item Bộ đếm tuần tự phù hợp với ngưỡng nhỏ và cấu trúc tiền tố.
  \item Cây tổng/mạng đếm thuận lợi khi cần nhiều cận hoặc giải tăng dần.
  \item Ràng buộc đúng \(K\) nên dùng chung một cấu trúc tổng nếu có thể.
\end{enumerate}

\subsection*{Câu hỏi nghiên cứu}
\begin{enumerate}
  \item Dựng bộ đếm tuần tự cho \(n=5,k=2\) và liệt kê các thanh ghi.
  \item So sánh EXK bằng hai bộ đếm độc lập với một cây tổng chung.
