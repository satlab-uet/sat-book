...
\begin{enumerate}
  \item \emph{pairwise encoding} (mã hóa từng cặp) và các phép phân rã AMO ưu tiên
        phát hiện xung đột trực tiếp;
  \item \emph{sequential counter} (bộ đếm tuần tự) vật hóa tổng trên từng tiền tố;
  \item \emph{totalizer} (cây tổng) vật hóa tổng theo cấu trúc cây;
  \item \emph{sorting/selection network} (mạng sắp xếp/lựa chọn) ưu tiên cấu
...
\emph{Product encoding} (mã hóa tích) đặt biến vào ma trận rồi dùng AMO theo
hàng và cột. \emph{Commander encoding} (mã hóa chỉ huy) chia biến thành nhóm,
...
\emph{Generalized totalizer} (cây tổng tổng quát) thay vectơ đơn phân liên tiếp
...
(biểu đồ quyết định đa trị, MDD), cây tổng
...
(phép mã hóa có xét cấu trúc): trước khi chọn bộ mã hóa cho một biểu thức
