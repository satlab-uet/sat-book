\chapter{Shared Counter cho các cửa sổ chồng lấn}
\chapterlead{Khi một ràng buộc trượt trên chuỗi, hai cửa sổ kề nhau chia sẻ gần
toàn bộ biến. Biểu diễn từng cửa sổ độc lập sẽ lặp trạng thái;
\emph{Shared Counter} (bộ đếm chia sẻ) biến phần giao đó thành một mô-đun
có thể tái sử dụng.}
...
Ràng buộc cửa sổ thuộc một họ lâu đời hơn các Shared Counter.
\textsc{Sequence}
yêu cầu mọi đoạn liên tiếp độ dài cố định có số biến nhận một giá trị trong
khoảng \([\ell,u]\). Dạng ràng buộc này xuất hiện trong bài toán lập lịch ca, xếp thứ tự xe, và xác định
...
\begin{description}[style=nextline]
  ...
  \(\ell\le S_{i+w}-S_i\le u\). Mọi cửa sổ chia sẻ một dãy trạng thái tiền tố.

  \item[Ôtômát/ràng buộc chính quy]
  Trạng thái ghi đủ lịch sử cần thiết để quyết định ký hiệu tiếp theo. Một
  ràng buộc \textsc{Regular} có thể mô hình hóa bài toán kiểm tra chuỗi thuộc một ngôn ngữ
...
So sánh có hệ thống các phép biểu diễn \textsc{Sequence} cho thấy cách phân rã,
...
\toprule
Họ & Trạng thái chia sẻ & Điểm mạnh & Chi phí/rủi ro\\
\midrule
Mỗi cửa sổ & Không hoặc ít & Đơn giản, dùng bộ biểu diễn sẵn & Lặp trạng thái phụ\\
...
Luồng & Nút và cung & Lọc toàn cục & Bộ lan truyền/phép biểu diễn phức tạp\\
...
Shared Counter theo khối của chương này nằm giữa hai cực. Phương pháp không tạo một
...
Nếu mỗi cửa sổ dùng một Sequential Counter riêng, kích thước xấp xỉ
\[
  O((n-w+1)wk).
\]
Trong khi đó, hai cửa sổ kề nhau chỉ khác một biến đi ra và một biến đi vào.
Mục tiêu của phép biểu diễn chia sẻ là đưa chi phí về gần
\[
  O(nk)
\]
...
cửa sổ giao biên giữa \(B_b,B_{b+1}\) là hợp của một hậu tố bên trái và một
...
\begin{tikzpicture}[satfig,x=11mm,y=9mm]
  ...
  \draw[satedge] (f)--(c);
\end{tikzpicture}
\caption{Một cửa sổ giao biên được phân rã thành hậu tố của khối trái và tiền tố
...
Điều kiện at-most-\(k\) tương đương với điều kiện không tồn tại
...
\(3\). Ví dụ cho thấy bộ nối không cộng lại toàn cửa sổ; mô-đun chỉ ghép hai
bằng chứng ngưỡng.
\end{workedexample}

\section{Bộ nối như một giao diện}

Bộ đếm cục bộ không phụ thuộc vào cửa sổ tiêu thụ; mô-đun chỉ cung cấp đặc tả của các
literal ngưỡng. Bộ nối biết cách một ràng buộc giao biên và chọn các ngưỡng cần
ghép.
...
Với \(k=1\), mỗi cửa sổ thỏa AMO. \emph{Shared Counter Ladder}
(thang Shared Counter, SCL) xây các trạng thái AMO/AMZ trong khối và
nối chúng qua các biên. So với phương án tạo xung đột từng cặp trong từng cửa sổ,
...
Biểu diễn Duplex dùng BDD tiến/lùi trong mỗi cửa sổ và liên kết các trạng thái
...
chuyển và phần giao lớn giữa các khoảng kề nhau. Biểu diễn từng cặp và bộ đếm tuần
...
phép biểu diễn từng cửa sổ. Khi đưa vào bài toán Anti-Bandwidth, SCL đạt kết quả
...
Khi \(k>1\), mỗi khối cần nhiều mức ngưỡng. Ta dùng Sequential Counter:
\[
  F^b_{i,q}
  \leftrightarrow
  F^b_{i-1,q}\lor
...
Ladder AMK trong \textcite{truong2026ladderamk} phát triển Sequential Counter dùng
...
một khoảng; các biên khoảng thay đổi đơn điệu theo hàng. Biểu diễn tổng tiền tố
dùng một trạng thái toàn cục cho mọi cột. Ladder AMK chỉ hiện thực hóa (reification) những ngưỡng
...
trạng thái tăng theo tổ hợp các bộ đếm đang hoạt động. Số ngưỡng chia sẻ tăng
...
buộc tách biệt nhưng chia sẻ đại lượng trung gian. Đây là câu hỏi thực
nghiệm và lý thuyết mang nhiều ý nghĩa hơn so với việc chỉ so sánh số lượng mệnh đề trên một cấu hình.

\section{Đúng đắn}

\begin{theorem}[Tính đúng của phép biểu diễn cửa sổ chia sẻ]
...
\begin{enumerate}
  ...
  \item với cửa sổ \(A\cup B\) giao biên, phép biểu diễn chứa đủ các mệnh đề
  \[
    \neg T(A,q)\lor\neg T(B,k+1-q),
    \qquad q=0,\ldots,k+1.
  \]
...
vượt \(k\). Nếu giao biên, đặt \(a=\sum_{x\in A}x\),
...
lệ trạng thái chia sẻ. Điều kiện ``mỗi cửa sổ cắt nhiều nhất một biên'' do
...
sổ là một khoảng, chi phí của một biên bằng số ràng buộc giao biên nhân với số
ngưỡng cần nối.

\section{Triển khai}

\emph{Application programming interface} (giao diện lập trình, API) cho
mô-đun có thể là:
\[
...
\begin{algorithm}[H]
\caption{Sinh AMK cửa sổ chia sẻ khi mỗi cửa sổ cắt nhiều nhất một biên}
...
\subsection{Miền hiệu quả của Shared Counter}

Shared Counter đạt lợi thế rõ nhất khi các cửa sổ chồng lấn mạnh, cận \(k\)
nhỏ và mỗi cửa sổ chỉ cắt ít khối. Với các cửa sổ gần rời nhau hoặc bài toán
nhỏ, biểu diễn từng cặp và bộ đếm độc lập thường có hằng số thấp hơn. Một bộ chọn
...
Ngoài các ràng buộc cửa sổ trượt \textsc{Sequence}, nguyên lý tái sử dụng trạng thái phụ qua các điểm cắt miền còn được mở rộng thành kỹ thuật hậu tố thưa \mbox{\textnormal{\textsc{SparseSuffix}}} trong bài toán lập lịch cuộc họp doanh nghiệp (B2B Meeting Scheduling) \parencite{nguyen2026compactsatmaxsatencodings} (xem Mục~8.5). Tại đó, các biến đại diện hậu tố được chia sẻ trực tiếp giữa các quan hệ thứ tự tiên quyết có cùng điểm cắt miền tiền nhiệm, giúp giảm đáng kể số mệnh đề nhị phân sinh ra trong công thức \CNF.
...
\begin{enumerate}
  ...
  \item Thanh ghi tiến và lùi tách một cửa sổ giao biên thành hai phần.
  \item Chứng minh tính đúng tách cửa sổ nội khối khỏi cửa sổ giao biên.
...
\begin{enumerate}
  \item Dựng các bộ nối cho một cửa sổ AMK với \(k=2\) giao biên thành hai phần.
  \item Tính số thanh ghi khi \(n=20,w=5,k=3\) và độ rộng khối bằng \(5\).
  \item Đề xuất hàm mục tiêu cho phân hoạch thích nghi dựa trên số cửa sổ giao biên.
\end{enumerate}
