...
\chapterlead{Một phép mã hóa tốt không chỉ mô tả nghiệm; nó còn quyết định cách
bộ giải nhìn thấy các nghiệm tương đương và các xung đột cục bộ. Phá đối xứng
(\emph{symmetry breaking}) thu gọn không gian tìm kiếm, còn liên kết biểu diễn
(\emph{channeling}) nối các góc nhìn bổ sung của cùng một quyết định.}
...
đại diện của các quỹ đạo còn lại, còn thí nghiệm bóc tách đo tác động của từng
lớp lên hiệu năng
...
phá đối xứng nhỏ, rẻ và phù hợp với mô hình thường hữu ích hơn việc cố mã hóa
...
Trong bài toán đóng gói, biến chiếm chỗ theo từng ô có thể tạo nhiều cách diễn
...
\]
ràng buộc \(A\le_{\mathrm{lex}}B\) chọn thứ tự từ điển. Một phép mã hóa dùng
các biến bằng nhau trên tiền tố \(e_i\):
\[
  e_i\leftrightarrow e_{i-1}\land(a_i\leftrightarrow b_i),
  \qquad e_0=\top,
\]
và cấm \(e_{i-1}\land a_i\land\neg b_i\). Ràng buộc dẫn đầu theo thứ tự từ
điển (\emph{lex-leader}) mạnh hơn tiền tố sử dụng nhưng tạo thêm một chuỗi biến
...
\[
  A\lor B\lor C\lor D.
\]
Trong đóng gói, hai hình \(i,j\) không chồng lấn khi ít nhất một quan hệ đúng:
\[
...
Biến quan hệ đóng vai trò biến chứng cho một nhánh của phép tuyển. Chúng rút
ràng buộc toàn cục thành các mệnh đề cục bộ quanh cặp đối tượng. Khi một quan
hệ được chọn, nó kích hoạt một quan hệ kéo theo trên tọa độ hoặc thời gian; khi
miền tọa độ loại quan hệ đó, mệnh đề tuyển buộc bộ giải chọn quan hệ còn lại.

\subsection{Biến quan hệ và giá trị hỗ trợ}

Từ góc nhìn chuyển CSP sang SAT, biến quan hệ nằm giữa mã hóa trực tiếp và mã
hóa hỗ trợ. Nó không chọn toàn bộ giá trị của hai biến; nó chọn một
...
\begin{tikzpicture}[satfig,x=8mm,y=7mm]
  \foreach \x/\y/\lab in
    {0/2/\(L_{ij}\),5/2/\(L_{ji}\),0/-1/\(B_{ij}\),5/-1/\(B_{ji}\)}{
    \node[satlabel] at (\x+1.6,\y+1.0) {\lab};
  }
...
Một literal quan hệ làm biến chứng cho cả một lớp cặp tọa độ.}
\label{fig:four-spatial-relations}
\end{figure}

\section{Mã hóa thứ tự và các literal so sánh}

Cho biến nguyên \(z\in\{0,\ldots,U\}\). Mã hóa thứ tự dùng
\[
  o_{z,t}=1
...
trúc dịch chuyển này có tính địa phương cao và phù hợp với bài toán đóng gói
...
\begin{workedexample}{Liên kết mã hóa trực tiếp--thứ tự cho một giá trị nguyên}
Cho \(z\in\{0,1,2,3,4\}\). Mã hóa trực tiếp có \(d_a\Leftrightarrow(z=a)\);
mã hóa thứ tự có \(o_t\Leftrightarrow(z\ge t)\). Hai góc nhìn được nối bởi
\[
...
\]
Chiều đầu đủ cho tính đúng nếu \(q\) chỉ được dùng làm biến chứng được chọn:
...
\begin{itemize}
  \item Nếu mệnh đề \(q_1\lor\cdots\lor q_r\) yêu cầu một biến chứng và không
  nơi nào đọc \(\neg q\), quan hệ kéo theo tiến thường đủ.
  \item Nếu \(\neg q\) được dùng để kết luận quan hệ đối nghịch, phải có
  đặc tả mạnh hơn.
  \item Nếu nhiều ràng buộc và hàm mục tiêu dùng chung \(q\), quan hệ tương
  đương giúp
...
Phép mã hóa kết hợp tạo một vòng suy luận:
\[
...
tạo được chúng. Đặc tả này có thể kiểm bằng các phép gán bộ phận và lan truyền
đơn vị như trong Chương 2.

Việc giữ nhiều góc nhìn cũng liên quan đến mã hóa lại có cấu trúc: biến phụ có
giá trị khi nó đặt tên cho một lớp hỗ trợ được nhiều ràng buộc dùng lại, thay
...
\end{tikzpicture}
\caption{Vòng liên kết trong phép mã hóa kết hợp. Thiếu một chiều có thể làm
...
\[
  ...
  \end{cases}
\]
Các literal quan hệ được dùng chung cho hai hướng đặt và được liên kết với phép
...
\]
Nó cũng cho phép tái sử dụng cùng biến \(\rho_i\) trong cận miền, quy tắc trội
và bộ giải mã.
...
và miền thời gian của chúng giao nhau. Trong đóng gói, mọi cặp vật cùng khung
...
cụ thể về mức độ phù hợp với bộ giải, thay vì chỉ nói phép mã hóa ``gọn''.
...
\begin{enumerate}
  ...
  \item thêm ràng buộc đếm hoặc bộ đếm dùng chung cho tài nguyên;
  \item thêm các ràng buộc phá đối xứng có chứng minh quỹ đạo;
  \item gắn hàm mục tiêu bằng các giả định hoặc mệnh đề mềm.
\end{enumerate}

Các nghiên cứu về lập lịch, đóng gói dải/khung chứa và Social Golfer trong
...
\begin{enumerate}
  ...
  \item Biến quan hệ là biến chứng có ngữ nghĩa, giúp cục bộ hóa phép tuyển.
  \item Liên kết mạnh đến đâu phụ thuộc vào cách biến quan hệ được đọc.
  \item Mã hóa thứ tự làm các cận dịch chuyển trở thành mệnh đề đơn giản.
...
\begin{enumerate}
  ...
  \item Cho một ràng buộc phản xạ trong bài toán đóng gói dải và nêu điều kiện
